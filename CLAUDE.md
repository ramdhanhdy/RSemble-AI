...
9. **Contextual Compare Results and task promotion (Child 05):** Compare owns a `ComparisonResultIndex` (`id == runId`) with task binding, lineage, and evidence receipt revision. Routes `/compare` and `/compare/results/:comparisonId` reconstruct results from persisted state on reload. Pre-call persistence creates the index and input snapshot before any provider call. Promotion uses exact-match validation only — semantic similarity never creates identity. Evidence receipt reuses Child 04 `EvidenceReceipt`. Legacy migration creates idempotent indexes for existing full Compare records. The archive v2 envelope gains Comparison Result indexes with collision-abort-before-write import.
