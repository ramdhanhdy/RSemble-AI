  ...
  for (let i = 0; i < 40; i++) {
    try {
      const list = await new Promise((res, rej) => {
        http
          .get("http://127.0.0.1:9337/json/list", (r) => {
            let d = "";
            r.on("data", (c) => (d += c));
...
console.log(
  "A11Y conn open focus-in-dialog:",
  await evalJs(`document.querySelector('[role=dialog]')?.contains(document.activeElement)`),
);
for (let i = 0; i < 30; i++) await key("Tab", "Tab", 9);
console.log(
  "A11Y conn trap-after-30-tab:",
  await evalJs(`document.querySelector('[role=dialog]')?.contains(document.activeElement)`),
);
await key("Escape", "Escape", 27);
await wait(400);
console.log(
  "A11Y conn escape-closes:",
  await evalJs(`document.querySelector('[role=dialog]') === null`),
);
console.log(
  "A11Y conn focus-restored:",
  await evalJs(`document.activeElement === window.__trigger`),
);
...
console.log(
  "A11Y palette open:",
  await evalJs(
    `document.querySelector('[role=dialog]') !== null || document.querySelector('[role=listbox]') !== null || !!document.querySelector('[cmdk-root]')`,
  ),
);
await key("Escape", "Escape", 27);
await wait(400);
...
console.log(
  "A11Y drawer open focus-in-dialog:",
  await evalJs(`document.querySelector('[role=dialog]')?.contains(document.activeElement)`),
);
for (let i = 0; i < 30; i++) await key("Tab", "Tab", 9);
console.log(
  "A11Y drawer trap-after-30-tab:",
  await evalJs(`document.querySelector('[role=dialog]')?.contains(document.activeElement)`),
);
...
await wait(400);
console.log(
  "A11Y drawer escape-closes:",
  await evalJs(`document.querySelector('[role=dialog]') === null`),
);
console.log(
  "A11Y drawer focus-restored:",
  await evalJs(`document.activeElement === window.__trigger`),
);
