...
describe("handleNineRouterProxy — forwarding", () => {
  it("maps GET /9router/v1/models to <upstream>/v1/models preserving query", async () => {
    ...
    const res = makeRes();
    await handleNineRouterProxy(makeReq("GET", undefined, {}), res, "/9router/v1/models?limit=50", {
  ...
  it("forwards POST /9router/v1/chat/completions and streams response bytes", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(upstreamWithChunks(["data: hello\n\n", "data: world\n\n"]));
    vi.stubGlobal("fetch", fetchMock);
    const res = makeRes();
...
describe("handleNineRouterProxy — authorization", () => {
  it("omits the Authorization header upstream when incoming auth is blank", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    const res = makeRes();
    await handleNineRouterProxy(makeReq("GET", undefined, {}), res, "/9router/v1/models", {
      upstream: "http://127.0.0.1:20128",
    });
...
describe("handleNineRouterProxy — abort propagation", () => {
  it("passes an AbortSignal to the upstream fetch", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    const res = makeRes();
    await handleNineRouterProxy(makeReq("POST", "{}"), res, "/9router/v1/chat/completions", {
      upstream: "http://127.0.0.1:20128",
    });
    const init = fetchMock.mock.calls[0][1] as RequestInit;
...
describe("handleNineRouterProxy — SSE clean-EOF normalization", () => {
  it("separates and terminates a final content data line that has no trailing newline", async () => {
    const unterminated = `data: ${JSON.stringify({ choices: [{ delta: { content: "OK" } }] })}`;
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(upstreamWithChunks([unterminated])));
    const res = makeRes();
    await handleNineRouterProxy(
    ...
    expect(body).toContain(`${unterminated}\n\ndata: [DONE]\n\n`);
  });
  ...
  it("appends [DONE] after a content-bearing stream that ends without the sentinel", async () => {
    const fetchMock = vi.fn().mockResolvedValue(upstreamWithChunks([sseDelta("OK")]));
    vi.stubGlobal("fetch", fetchMock);
    const res = makeRes();
    await handleNineRouterProxy(
      makeReq("POST", JSON.stringify({ stream: true })),
  ...
  it("does not duplicate [DONE] when the upstream already sent it", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(upstreamWithChunks([sseDelta("OK"), "data: [DONE]\n\n"]));
    vi.stubGlobal("fetch", fetchMock);
    const res = makeRes();
    await handleNineRouterProxy(
      makeReq("POST", JSON.stringify({ stream: true })),
  ...
  it("does not append [DONE] to an empty stream with no content", async () => {
    const fetchMock = vi.fn().mockResolvedValue(upstreamWithChunks([""]));
    vi.stubGlobal("fetch", fetchMock);
    const res = makeRes();
    await handleNineRouterProxy(
      makeReq("POST", JSON.stringify({ stream: true })),
  ...
  it("handles data: and JSON split across chunks", async () => {
    ...
    const fetchMock = vi
      .fn()
      .mockResolvedValue(upstreamWithChunks([event.slice(0, mid), event.slice(mid)]));
    vi.stubGlobal("fetch", fetchMock);
    const res = makeRes();
    await handleNineRouterProxy(
      makeReq("POST", JSON.stringify({ stream: true })),
