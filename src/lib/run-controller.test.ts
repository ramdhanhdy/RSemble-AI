  ...
  const dispatch: React.Dispatch<Action> = (a) => {
    ...
    if (a.type === "INSUFFICIENT_CANDIDATES")
      stateRef.current = {
        ...stateRef.current,
        running: false,
        insufficient: { done: a.done, failed: a.failed },
      };
...
describe("run-controller — guarded paths", () => {
  it("runFanout with no enabled slots is blocked before any provider call", async () => {
    ...
    expect(dispatched).toContainEqual({
      type: "FANOUT_BLOCKED",
      reason: "Enable at least two candidate models.",
    });
  ...
  it("runFanout with exactly one enabled slot is blocked before any provider call", async () => {
    ...
    expect(dispatched).toContainEqual({
      type: "FANOUT_BLOCKED",
      reason: "Add or enable one more candidate to compare.",
    });
    expect(chatStreamMock).not.toHaveBeenCalled();
  });
    ...
    state.candidates = [
      {
        ...
        scores: {},
        weightedScore: 0,
        segments: [{ id: "a", text: "existing answer" }],
        status: "done",
        startedAt: 100,
        finishedAt: 200,
      },
  ...
  it("re-fuse builds the request from one consistent persisted record snapshot", async () => {
    // A re-Fuse (triggerFusion after a completed fuse run) must load the
    // persisted record exactly once and derive the accepted candidate attempt
    // IDs, the accepted Judge attempt ID, and the Judge's blind-label map from
    // that single snapshot — never from two separately-fetched records that
    // could have diverged (Plan 007).
    const state = stateWithSlots(TWO_SLOTS, "fuse");
    state.judgeStatus = "done";
    state.judgeReport = { labelMap: [], evaluationsById: {}, comparisons: [] };
    const recorder = makeRecorderSpies();
    // One frozen snapshot: candidate accepted attempt IDs + a Judge with an
    // accepted attempt that carries a blind-label map.
    const frozenJudgeAttemptId = "judge-att-frozen";
    const record = {
      id: "run-1",
      revision: 1,
      candidates: [
        { candidateId: "cand-s1", acceptedAttemptId: "cand-att-1" },
        { candidateId: "cand-s2", acceptedAttemptId: "cand-att-2" },
      ],
      judge: {
        status: "completed",
        acceptedAttemptId: frozenJudgeAttemptId,
        attempts: [
          {
            attemptId: frozenJudgeAttemptId,
            blindLabelToCandidateId: { A: "cand-s1", B: "cand-s2" },
          },
        ],
      },
    };
    recorder.getRecord.mockResolvedValue(record as never);
    ...
    const controller = createRunController(deps);
    // First run: executes fanout + judge + fusion, setting runIdRef.
    await controller.runFanout();
    expect(recorder.beginFusionAttempt).toHaveBeenCalledTimes(1);

    // Re-fuse from the persisted record.
    ...
    const fusionStartInput = fusionAttemptInputs[fusionAttemptInputs.length - 1];
    // The request references the frozen Judge attempt and candidate attempts
    // from that single snapshot.
    expect(fusionStartInput?.sourceJudgeAttemptId).toBe(frozenJudgeAttemptId);
    expect(fusionStartInput?.candidateAttemptIdsByCandidateId).toEqual({
      "cand-s1": "cand-att-1",
      "cand-s2": "cand-att-2",
    });
  });
...
describe("run-controller — judge instruction threading", () => {
  it("passes state.judgeInstruction into the judge prompt (rank path)", async () => {
    ...
    chatStreamMock.mockImplementation(() => streamOf("some answer"));
    chatCompletionMock.mockResolvedValue(judgeResponse([]));
    const { deps } = makeDeps(state);
    await createRunController(deps).runFanout();
...
describe("run-controller — partial candidate failures", () => {
    ...
    state.candidates = [
      {
        ...
        scores: {},
        weightedScore: 0,
        segments: [{ id: "a", text: "real answer" }],
        status: "done",
      },
      {
        id: "cand-s2",
  ...
  it("triggerFusion fuses when ≥2 usable candidates exist (successful partial fusion via button)", async () => {
    ...
    state.candidates = [
      ...
      },
      {
        ...
        segments: [],
        status: "error",
        errorMessage: "gemini provider crashed",
      },
    ];
  ...
  it("triggerFusion is a no-op while a run is in progress (no provider call, no dispatch)", async () => {
    ...
    const fusionDispatches = dispatched.filter(
      (a) =>
        a.type === "FUSION_START" ||
        a.type === "INSUFFICIENT_CANDIDATES" ||
        a.type === "FUSION_RESULT",
    );
...
describe("run-controller — judge contract failures", () => {
  ...
  it("dispatches JUDGE_FAILED in rank mode when judge returns evaluations with missing candidates", async () => {
    const state = stateWithSlots(TWO_SLOTS, "rank");
    chatStreamMock.mockImplementation(() => streamOf("some answer"));
    chatCompletionMock.mockResolvedValue(judgeResponse([["A", 4.0]]));
    const { deps, dispatched, stateRef } = makeDeps(state);
    const controller = createRunController(deps);
    await controller.runFanout();
...
describe("run-controller — retryJudge (Judge-only recovery)", () => {
  it("uses the captured Fuse mode when a legacy context omitted mode", async () => {
    ...
    expect(dispatched).toContainEqual(
      expect.objectContaining({ type: "JUDGE_RESULT", mode: "fuse" }),
    );
  });
  ...
  it("leaves candidate text and metadata byte-for-byte unchanged", async () => {
    ...
    const mutatingTypes = new Set([
      ...
      "RETRY_CANDIDATE_RESULT",
      "RETRY_CANDIDATE_FAILED",
      "RETRY_CANDIDATE_DELTA",
    ]);
  ...
  it("dispatches INSUFFICIENT_CANDIDATES and makes no Judge call with fewer than two usable candidates", async () => {
    ...
    state.candidates = [
      doneCandidate("cand-1", "openrouter", "model-a", "answer from model A"),
      {
        ...doneCandidate("cand-2", "umans", "model-b", ""),
        status: "error",
        errorMessage: "boom",
        segments: [],
      },
...
describe("run-controller — characterization (pre-extraction invariants)", () => {
  ...
  it("Judge retry makes zero candidate stream calls", async () => {
    const state = stateWithSlots(TWO_SLOTS, "rank");
    chatStreamMock.mockImplementation(() => streamOf("answer"));
    chatCompletionMock.mockResolvedValueOnce("malformed judge output").mockResolvedValueOnce(
      judgeResponse([
        ["A", 4],
        ["B", 3],
      ]),
...
describe("retryCandidate — frozen attachment set from runContext (7.6.6)", () => {
  ...
  it("streams the retry with the ORIGINAL attachments even if the user changed state", async () => {
    ...
    const state: StudioState = {
      ...
      candidates: [failed, doneCandidate("cand-s2", "umans", "model-b", "answer from model B")],
      runContext: {
    ...
    const messages = (
      chatStreamMock.mock.calls[0][0] as { messages: { content: string | unknown[] }[] }
    ).messages;
...
describe("run-controller — cross-tab paid execution lease", () => {
  it("rejects a conflicting live lease before any candidate provider call", async () => {
    ...
    expect(dispatched).toContainEqual(
      expect.objectContaining({
        type: "FANOUT_BLOCKED",
        reason: expect.stringContaining("Another execution"),
      }),
    );
    expect(chatStreamMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects a late candidate result after tab B reclaims before tab A's next heartbeat", async () => {
    ...
    const tabA = new InMemoryExecutionLease(shared, channelA as never, {
      ownerId: "tab-a",
      now,
      ttl: 10_000,
    });
    const tabB = new InMemoryExecutionLease(shared, channelB as never, {
      ownerId: "tab-b",
      now,
      ttl: 10_000,
    });
