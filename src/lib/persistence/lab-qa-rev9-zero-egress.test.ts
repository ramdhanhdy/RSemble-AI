...
function freshDb(name: string): RSembleEvaluationDB {
  const db = new RSembleEvaluationDB(`rev9-${name}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
  testDbs.push(db);
  return db;
}
...
describe("REV-9 — migration operations cause zero provider egress", () => {
  ...
  it("the v12 → v13 cutover upgrade makes zero provider/fetch/XHR calls", async () => {
    ...
    v12.version(3).stores({ tasks: "id", taskVersions: "[taskId+version]", taskArtifacts: "id", taskArtifactBytes: "id", taskInstances: "id", taskFamilies: "id", taskFamilyAssignments: "id", taskFacetAnnotations: "id", taskMigrationCrosswalk: "legacyScopeKey" });
    ...
    v12.version(8).stores({ modelConfigurations: "id", observations: "id", evidenceDecisions: "id", evidenceIndexJobs: "sourceResultId" });
    ...
    for (const s of FUSION_CORPUS_FIXTURE.studies) {
      await v12.table("fusionStudies").put({ id: s.id, study: s, revision: s.revision, suiteId: "suite-default", suiteVersion: 1, status: s.status, updatedAt: s.updatedAt });
    }
    v12.close();
