...
function makeExperiment(
  ...
  const snapshot: ExperimentSnapshot = {
    ...
    modelSlots: [
      ...
      { id: "s2", providerId: "umans", provider: "Umans", model: "Model2", slug: "model2", enabled: true },
      { id: "s3", providerId: "9router", provider: "9Router", model: "Route", slug: "route", enabled: true },
      { id: "s4", providerId: "9router", provider: "9Router", model: "Route2", slug: "route2", enabled: true },
      { id: "s5", providerId: "9router", provider: "9Router", model: "Route3", slug: "route3", enabled: true },
      { id: "s6", providerId: "9router", provider: "9Router", model: "Route4", slug: "route4", enabled: true },
      { id: "s7", providerId: "9router", provider: "9Router", model: "Route5", slug: "route5", enabled: true },
      { id: "s8", providerId: "9router", provider: "9Router", model: "Route6", slug: "route6", enabled: true },
    ],
...
describe("buildTaskLedger", () => {
      ...
      [
        ...
        makeState(
          ...
          [
            ...
            makeAttempt("att-good", "completed", { trial: 2, coverage: makeCoverage(8, 8), startedAt: now - 30_000 }),
            makeAttempt("att-new", "failed", { trial: 3, startedAt: now - 10_000 }),
          ],
          "att-good",
        ),
        // Running attempt is the live state even without stored selection.
        makeState("t2", [makeAttempt("att-run", "running", { runId: "run-2", startedAt: now - 5_000 })]),
  ...
  it("falls back to selectAttemptId policy, then the newest attempt, for display status", () => {
    ...
    const experiment = makeExperiment(
      ...
      [
        ...
        makeState("t1", [
          makeAttempt("att-p1", "partial", { coverage: makeCoverage(4, 8), startedAt: now - 20_000 }),
          makeAttempt("att-p2", "partial", { coverage: makeCoverage(6, 8), startedAt: now - 10_000 }),
        ]),
  ...
  it("counts complete, partial, failed, and queued rows (failed includes aborted/interrupted)", () => {
    const experiment = makeExperiment(
      [makeTask("t1", "Alpha", 0), makeTask("t2", "Beta", 1), makeTask("t3", "Gamma", 2), makeTask("t4", "Delta", 3), makeTask("t5", "Epsilon", 4)],
      [
        makeState("t1", [makeAttempt("a1", "completed", { coverage: makeCoverage(8, 8) })]),
  ...
  it("exposes titles, coverage fallback to suite slot count, and full attempt history", () => {
    const experiment = makeExperiment(
      [makeTask("t1", "Alpha", 0)],
      [makeState("t1", [makeAttempt("a1", "completed", { trial: 2, coverage: makeCoverage(6, SLOT_COUNT) })])],
    );
...
describe("filterTaskLedgerRows", () => {
  ...
  it("queued and complete select exactly their status", () => {
    expect(filterTaskLedgerRows(rows, "queued", "running").map((r) => r.taskId)).toEqual(["t-queued"]);
    expect(filterTaskLedgerRows(rows, "complete", "running").map((r) => r.taskId)).toEqual(["t-complete"]);
  });
...
describe("pageTaskLedgerRows", () => {
  ...
  it("handles empty and underfull lists", () => {
    expect(pageTaskLedgerRows([], 1)).toMatchObject({ rows: [], page: 1, pageCount: 1, start: 0, end: 0, total: 0 });
